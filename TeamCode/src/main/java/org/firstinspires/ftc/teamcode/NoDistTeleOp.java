...
//@Disabled
public class NoDistTeleOp extends LinearOpMode {
...
//
            ...
            else if(gamepad1.left_bumper && armStage == 2 && liftTimer.seconds() > 0.5){
                armStage = 1;
                liftTimer.reset();
            }
            else if(gamepad1.left_bumper && armStage == 3 && liftTimer.seconds() > 0.5){
                armStage = 2;
                liftTimer.reset();
            }
            else if(gamepad1.left_bumper && armStage == 4 && liftTimer.seconds() > 0.5){
                armStage = 3;
                liftTimer.reset();
            }
//
            if(gamepad1.right_bumper && armStage == 0 && liftTimer.seconds() > 0.5){
                //Display right bumper has been pressed & armstage value
                armStage = 1;
                liftTimer.reset();
            }
            else if(gamepad1.right_bumper && armStage == 1 && liftTimer.seconds() > 0.5){
                armStage = 2;
                liftTimer.reset();
            }
            else if(gamepad1.right_bumper && armStage == 2 && liftTimer.seconds() > 0.5){
                armStage = 3;
                liftTimer.reset();
            }
            else if(gamepad1.right_bumper && armStage == 3 && liftTimer.seconds() > 0.5){
                armStage = 4;
                liftTimer.reset();
            }
...
//            }
            ...
            if (gamepad1.dpad_left & climbRight.getCurrentPosition() > 1000 & climbLeft.getCurrentPosition() > 1000) { //hanging hooks come down
                hookLeft.setPosition(0.32);
                hookRight.setPosition(0.38);
            }
