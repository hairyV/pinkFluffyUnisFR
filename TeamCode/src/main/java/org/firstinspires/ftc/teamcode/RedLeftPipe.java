...
public class RedLeftPipe extends OpenCvPipeline {
    Telemetry telemetry;
    ...
    public RedLeftPipe(Telemetry t) { telemetry = t;}

    @Override
    public Mat processFrame(Mat input) {

        Imgproc.cvtColor(input, mat, Imgproc.COLOR_RGB2HSV); //converting rgb to hsv codec
        Scalar lowHSV = new Scalar(0,100,54); //darker reds
        Scalar highHSV = new Scalar(15,255,255); //lighter reds
...
//        //Scalar highHSV = new Scalar(130, 255, 255);
        ...
        */
        if (barcodeLeft) {
            //left
            location = RedLeftPipe.Location.LEFT;
            telemetry.addData("Element Location", "left");
        } else if (barcodeMiddle) {
            //middle
            location = RedLeftPipe.Location.MIDDLE;
            telemetry.addData("Element Location", "middle");
        } else {
            //right
            location = RedLeftPipe.Location.RIGHT;
            telemetry.addData("Element Location", "right");
        }
        telemetry.update();
